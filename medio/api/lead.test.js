...
function mockarFetch(chamadas, { syncOk = true, listaOk = true, tlOk = true } = {}) {
  global.fetch = async (url, opts) => {
    ...
    if (String(url).includes('/api/3/tags?')) return resp(true, { tags: [{ id: '4242', tag: 'LP Bolsão Ensino Médio - ZH+ 2027' }] });
...
(async () => {
  {
    ...
    assert.strictEqual(campos['12'], '2ª Série - Ensino Médio', 'campo 12 deve levar a turma');
    assert.strictEqual(campos['72'], 'Maria Clara Souza', 'campo 72 deve levar o nome do aluno');
    assert.strictEqual(campos['46'], 'Ensino Médio', 'segmento fixo da LP do Médio');
  ...
  {
    /* Esta LP é só do Ensino Médio: turma de outro segmento não passa, mesmo
       existindo na unidade e no ActiveCampaign. */
    const { res, chamadas } = await executar({ ...leadValido, turma: 'Infantil N3' });
    assert.strictEqual(res.code, 400, 'turma fora do Ensino Médio deveria ser recusada');
    ...
    console.log('ok  turma fora do Médio é recusada antes de tocar o CRM');
  }

  {
    const { res } = await executar({ ...leadValido, unidade: 'Niterói' });
    assert.strictEqual(res.code, 400, 'unidade fora do ZH+ deveria ser recusada');
    console.log('ok  unidade inexistente é recusada');
  }

  {
    /* As três séries valem nas três unidades, como a página promete. */
    for (const unidade of ['Icaraí', 'Méier', 'Vila Isabel']) {
      for (const turma of ['1ª Série - Ensino Médio', '2ª Série - Ensino Médio', '3ª Série - Ensino Médio']) {
        const { res } = await executar({ ...leadValido, unidade, turma });
        assert.strictEqual(res.code, 200, `${turma} em ${unidade} deveria ser aceita`);
      }
    }
    console.log('ok  as 9 combinações de série e unidade são aceitas');
  }
...

  {
    ...
    assert.strictEqual(tl.headers.Origin, 'https://bolsao.zhmais.com.br', 'CRM confere a origem');
    ...
    assert.deepStrictEqual(tl.corpo['form-field-field_009575c'], ['2ª Série - Ensino Médio'], 'Turma vai como array');
    assert.strictEqual(tl.corpo['form-field-field_0d0c50e'], 'Maria Clara Souza', 'Aluno');
    assert.match(tl.corpo.external_id, /^lp-bolsao-medio-zhmais-2027-[0-9a-f-]{36}$/);
