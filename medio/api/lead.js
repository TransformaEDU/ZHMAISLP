/**
 * Recebe o cadastro da landing page do BOLSÃO ZH+ — ENSINO MÉDIO e grava o
 * lead em dois CRMs, em paralelo:
...
const SEGMENTO = 'Ensino Médio';                      // opção 1484 do campo 46

/* Séries aceitas por unidade. A lista existe para que um cliente adulterado
   não injete combinação inexistente no CRM, e os valores são exatamente os
   rótulos do campo "Turma (de interesse)" do ActiveCampaign (ids 66, 68 e 70)
   e das turmas da empresa Colégio ZH+ no TechLithy.

   DIVERGÊNCIA CONHECIDA: a planilha oficial de turmas lista, para o ZH+,
   1ª/2ª/3ª série só em Icaraí; no Méier, 1ª e 2ª; em Vila Isabel, só a 1ª.
   A página oferece as três séries nas três unidades. Aceitamos o que a página
   promete, porque recusar aqui perderia o lead de quem a própria página
   convidou. Quem confere a vaga é a secretaria da unidade. Se a planilha for a
   regra, é só remover as séries daqui e da lista data-uni do index.html. */
const SERIES = [
...
async function obterTagId(base, chave) {
  ...
  const criada = await chamarAC('/api/3/tags', {
    ...
    corpo: { tag: { tag: TAG_NOME, tagType: 'contact', description: 'Leads da landing page bolsao.zhmais.com.br (Bolsão Ensino Médio).' } },
  });
...
/* Contrato do webhook (Guia de integração de formulários do TechLithy CRM):
   ...
   Os leads vão para a empresa "Colégio ZH+" no CRM, fonte própria desta LP
   "LP Bolsão Médio ZH+ (bolsao.zhmais.com.br)" (TECHLITHY_ACCOUNT_ID é o uuid
...
const TL_PREFIXO = 'lp-bolsao-medio-zhmais-2027-';
const TL_ORIGEM = 'https://bolsao.zhmais.com.br';
